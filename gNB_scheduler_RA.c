...
#include "openair2/GNB_APP/MACRLC_nr_paramdef.h" //add: IITH
#include "openair2/GNB_APP/gnb_config.c" //add: IITH
                         ...
                         uint32_t preamble_power)
{
  ...
  if(msg1_display_enable==1) //<add:IITH>
...
//<add:IITH>
//LOG_I(NR_MAC,
//          "Scheduled retransmission frame count (Frame:%d, Slot:%d, sched_frame:%d,sched_slot:%d)\n",  
//          frame,
//          slot,
//          sched_frame,
//          sched_slot);
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    UE_iterator(mac->UE_info.access_ue_list, UE) {
      ...
      switch (ra->ra_state) {
        case nrRA_Msg2:
        //<add:IITH>
          if((fbs_enable==0 && msg1_drop_enable==0)||(fbs_enable==1 && msg1_drop_enable==0))
          {
              nr_generate_Msg2(module_idP, CC_id, frameP, slotP, UE, DL_req, TX_req); //IITH to stop the MSG2 transmission from the gNB or DU
              //printf("MACRLC_FBS_ENABLE_IDX %u\n",MACRLC_FBS_ENABLE_IDX);
              //printf("fbs_enable %u and msg1_drop_enable %u\n",fbs_enable,msg1_drop_enable);
              break;
          }
          else if((fbs_enable==0 && msg1_drop_enable==1)||(fbs_enable==1 && msg1_drop_enable==1))
          {
              //printf("fbs_enable %u and msg1_drop_enable %u\n",fbs_enable,msg1_drop_enable);
              break;
          }
        ...
        case nrRA_Msg3_retransmission:
          nr_generate_Msg3_retransmission(module_idP, CC_id, frameP, slotP, UE, ul_dci_req);  //IITH to stop the MSG3 retransmission
          break;
        case nrRA_Msg4:
        case nrRA_MsgB:
        //<add:IITH>
          if((fbs_enable==0 && msg3_drop_enable==0)||(fbs_enable==1 && msg3_drop_enable==0))
          {
              nr_generate_Msg4_MsgB(module_idP, CC_id, frameP, slotP, UE, DL_req, TX_req);
              //printf("MACRLC_FBS_ENABLE_IDX %u\n",MACRLC_FBS_ENABLE_IDX);
              //printf("fbs_enable %u and msg1_drop_enable %u\n",fbs_enable,msg3_drop_enable);
              break;
          }
          else if((fbs_enable==0 && msg3_drop_enable==1)||(fbs_enable==1 && msg3_drop_enable==1))
          {
              //printf("fbs_enable %u and msg1_drop_enable %u\n",fbs_enable,msg3_drop_enable);
              break;
          }
